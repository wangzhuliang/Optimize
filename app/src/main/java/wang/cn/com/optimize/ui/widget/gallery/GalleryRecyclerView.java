...
public class GalleryRecyclerView extends RecyclerView {
    ...
    public void onWindowFocusChanged(boolean hasWindowFocus) {
        ...
        if (mFirstHasWindowFocus) {
            if (hasRotate) {
                // 如果是横竖屏切换，不应该走smoothScrollToPosition(0)，
                // 因为这个方法会导致ScrollManager的onHorizontalScroll不断执行，
                // 而ScrollManager.mConsumeX已经重置，会导致这个值紊乱
                ...
                if (scrollPos > 1) {
                    // BUG FIX.滑动前两项时，横竖屏切换，
                    // 触发scrollToPosition(0)时ScrollManager.mConsumeX值为0;
                    // 而滑动超过两项时，ScrollManager.mConsumeX值不为0

...
public class ImageWatcher extends FrameLayout implements GestureDetector.OnGestureListener,
    ...
    /**图片加载失败站位图*/
    protected int mErrorImageRes = R.mipmap.error_picture;
    /**状态栏高度*/
    ...
    /**android手势检测，GestureDetector.OnGestureListener---接口*/
    ...
    public class DefaultIndexProvider implements IndexProvider {
        ...
        public View initialView(Context context) {
            ...
            // TypedValue.applyDimension(),非标准度量尺寸转变为标准度量尺寸 (Android系统中的标准尺寸是px, 即像素)
            float tCurrentIdxTransY = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
                    30, displayMetrics) + 0.5f;
            //设置这个控件相对于其顶部位置的垂直位置。这有效地定位在对象的布局后
    ...
    public interface OnPictureLongPressListener {
         ...
         */
        void onPictureLongPress(ImageView v, Uri uri, int pos);
    }
     ...
     * @param imageGroupList 被点击的ImageView的所在列表，
     *                       加载图片时会提前展示列表中已经下载完成的thumb图片
     * @param urlList        被加载的图片url列表，数量必须大于等于 imageGroupList.size。
     *                       且顺序应当和imageGroupList保持一致
     */
    public void show(ImageView i, SparseArray<ImageView> imageGroupList, final List<Uri> urlList) {
        ...
        if (initPosition < 0) {
            throw new IllegalArgumentException("param ImageView i must be a member of the List " +
                    "<ImageView> imageGroupList!");
        }
                ...
                if (vsDefault == null) {
                    ...
                    if ("horizontal".equals(imageOrientation)) {
                        ...
                        if (vsDefault.width * vsCurrent.scaleX <= mWidth) {
                            if (Math.abs(moveY) < mTouchSlop && Math.abs(moveX) > mTouchSlop
                                    && Math.abs(moveX) > Math.abs(moveY) * 2) {
                                mTouchMode = TOUCH_MODE_SLIDE;
                            }
                        } else {
                            float translateXRightEdge = vsDefault.width * vsCurrent.scaleX / 2 -
                                    vsDefault.width / 2;
                            float translateXLeftEdge = mWidth -
    ...
    public void onLongPress(MotionEvent e) {
        if (pictureLongPressListener != null) {
            pictureLongPressListener.onPictureLongPress(
                    iSource, mUrlList.get(vPager.getCurrentItem()), vPager.getCurrentItem());
        }
    }
    ...
    public class ImagePagerAdapter extends PagerAdapter {
        ...
        private final FrameLayout.LayoutParams lpCenter = new
        ...
        private boolean setDefaultDisplayConfigs(final ImageView imageView, final int pos,
            ...
            if (pos < mImageGroupList.size()) {
                ...
                if (bmpMirror != null) {
                    ...
                    ViewState vsThumb = ViewState.write(imageView, ViewState.STATE_THUMB).
                            width(bmpMirrorWidth).height(bmpMirrorHeight)
            ...
            loader.load(imageView.getContext(), mUrlList.get(pos), new LoadCallback() {
                ...
                public void onResourceReady(Drawable resource) {
                    ...
                    if (resourceImageWidth * 1f / resourceImageHeight > mWidth * 1f / mHeight) {
                        ...
                        sourceDefaultHeight = (int) (sourceDefaultWidth * 1f /
                                resourceImageWidth * resourceImageHeight);
    ...
    public boolean handleBackPressed() {
        return isInTransformAnimation || (iSource != null &&
                getVisibility() == View.VISIBLE && onSingleTapConfirmed());
    }
    ...
    private void animSourceViewStateTransform(ImageView view, final ViewState vsResult) {
        ...
        animImageTransform = ViewState.restoreByAnim(view, vsResult.mTag).
                addListener(mAnimTransitionStateListener).create();
        ...
        animBackground.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                ...
                if (stateChangedListener != null) {
                    stateChangedListener.onStateChangeUpdate(ImageWatcher.this,
                            iSource, getCurrentPosition(), getDisplayingUri(), p, tag);
                }
            }
        });
            ...
            public void onAnimationStart(Animator animation) {
                ...
                if (stateChangedListener != null) {
                    if (tag == STATE_ENTER_DISPLAYING) {
                        stateChangedListener.onStateChanged(ImageWatcher.this,
                                getCurrentPosition(), getDisplayingUri(), tag);
                    }
                }
            }

 ...
 * @time: 17:01
 */
public class MoveImageRelativeLayout extends RelativeLayout {
    ...
    public MoveImageRelativeLayout(Context context) {
        super(context, null);
    }

    public MoveImageRelativeLayout(Context context, AttributeSet attrs) {
        super(context, attrs, 0);
    }

    public MoveImageRelativeLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.MoveImageRelativeLayout);
        mRestorationRatio = ta.getFloat(R.styleable.MoveImageRelativeLayout_vdr_restoration_ratio, 0.1F);
        mStartOffsetRatioY = ta.getFloat(R.styleable.MoveImageRelativeLayout_vdr_start_offset_ratio_y, 0.5F);
        mStartAnimationEnable = ta.getBoolean(R.styleable.MoveImageRelativeLayout_vdr_start_anim_enable, true);
        mIsDragEnable = ta.getBoolean(R.styleable.MoveImageRelativeLayout_vdr_drag_enable, true);

        mAnimationDuration = ta.getInt(R.styleable.MoveImageRelativeLayout_vdr_anim_duration, 400);
        mTopNavHeight = ta.getDimension(R.styleable.MoveImageRelativeLayout_vdr_top_nav_height, 0);
        mBottomNavHeight = ta.getDimension(R.styleable.MoveImageRelativeLayout_vdr_bottom_nav_height, 0);
        ta.recycle();
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getAction() & ev.getActionMasked()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                mIsInterceptTouchEvent = Math.abs(dy) > mMinScaledTouchSlop |
                        Math.abs(dx) > mMinScaledTouchSlop;
                break;
        }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                mIsConsumeTouchEvent = !childInterceptEvent(this,
    ...
    private void startRestorationAnimation() {
        ...
        PropertyValuesHolder propertyScaleX = PropertyValuesHolder.
                ofFloat("scaleX", getScaleX(), 1.0F);
        PropertyValuesHolder propertyScaleY = PropertyValuesHolder.
                ofFloat("scaleY", getScaleY(), 1.0F);
        PropertyValuesHolder propertyTranslationX = PropertyValuesHolder.
                ofFloat("translationX", getTranslationX(), 0);
        PropertyValuesHolder propertyTranslationY = PropertyValuesHolder.
                ofFloat("translationY", getTranslationY(), 0);

        mRestorationAnimation = ObjectAnimator.ofPropertyValuesHolder(this,
                propertyScaleX, propertyScaleY, propertyTranslationX, propertyTranslationY)
    ...
    public MoveImageRelativeLayout setOnVideoDragListener(OnVideoDragListener listener) {
        mListener = listener;
        return this;
    }

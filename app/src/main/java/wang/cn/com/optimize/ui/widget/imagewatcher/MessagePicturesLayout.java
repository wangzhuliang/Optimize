...
public class MessagePicturesLayout extends FrameLayout implements View.OnClickListener{
    ...
    private final FrameLayout.LayoutParams lpChildImage = new
    ...
    public MessagePicturesLayout(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);

        DisplayMetrics mDisplayMetrics = context.getResources().getDisplayMetrics();
        mSingleMaxSize = (int) (TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
                216, mDisplayMetrics) + 0.5f);
        mSpace = (int) (TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
                2, mDisplayMetrics) + 0.5f);
    ...
    private void notifyDataChanged() {
        ...
        if (thumbList.size() > mDataList.size()) {
            throw new IllegalArgumentException("dataList.size(" +
                    mDataList.size() + ") > thumbDataList.size(" + thumbList.size() + ")");
        }
